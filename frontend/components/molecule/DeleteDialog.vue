...
async function handleDeleteOperation() {
  const res = await $axios.delete(`/tasks/${route.params.id}`, {
    headers: {
...
<template>
  <div>
    <TransitionRoot as="template" :show="deleteStore.status">
      <Dialog class="relative z-10" @close="deleteStore.close()">
        <TransitionChild
          as="template"
